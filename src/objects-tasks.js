...
function makeWord(lettersObject) {
  const arr = [];
  const arrValues = Object.entries(lettersObject);
...
const NOT_ALLOWED_ORDERS = [
  ORDER_OF_SELECTOR.ID,
  ORDER_OF_SELECTOR.ELEMENT,
  ORDER_OF_SELECTOR.PSEUDO_ELEMENT,
];
...
const cssSelectorBuilder = {
  ...
  element(value) {
    return this.createPartOfObject(value, ORDER_OF_SELECTOR.ELEMENT);
  },

  id(value) {
    return this.createPartOfObject(`#${value}`, ORDER_OF_SELECTOR.ID);
  },

  class(value) {
    return this.createPartOfObject(`.${value}`, ORDER_OF_SELECTOR.CLASS);
  },

  attr(value) {
    return this.createPartOfObject(`[${value}]`, ORDER_OF_SELECTOR.ATTR);
  },

  pseudoClass(value) {
    return this.createPartOfObject(`:${value}`, ORDER_OF_SELECTOR.PSEUDO_CLASS);
  },

  pseudoElement(value) {
    return this.createPartOfObject(
      `::${value}`,
      ORDER_OF_SELECTOR.PSEUDO_ELEMENT
    );
  },

  createPartOfObject(value, order) {
    if (this.order > order) {
      throw new Error(
        'Selector parts should be arranged in the following order: element, id, class, attribute, pseudo-class, pseudo-element'
      );
    }
    if (this.order === order && NOT_ALLOWED_ORDERS.includes(order)) {
      throw new Error(
  ...
  combine(selector1, combinator, selector2) {
    const result = `${selector1.result} ${combinator} ${selector2.result}`;
    return {
      result,
      stringify() {
        return result;
